...
from llm import FollowUpGenerator
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    """Normalise columns and recompute priority + bucket on the whole DataFrame."""
    df = df.copy()
...
with tab_ai:
    ...
    with col_r:
        if gen_btn:
            ...
            st.caption("✅ Saved to history for this session.")
...
with tab_analytics:
    ...
    with r1l:
        ...
        fig1 = px.bar(sc, x="Status", y="Count", color="Status",
    ...
    with r2l:
        ...
        color_map = {
            "Overdue": "#ef4444", "Due Today": "#f97316",
            "Upcoming": "#22c55e", "No Date": "#94a3b8",
        }
        fig3 = px.bar(bc, x="Bucket", y="Count", color="Bucket",
                      title="Follow-Up Urgency", color_discrete_map=color_map)
        fig3.update_layout(showlegend=False,
                           plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig3, use_container_width=True)

    with r2r:
        fig4 = px.histogram(df, x="priority_score", nbins=10,
                            title="Priority Score Distribution",
                            color_discrete_sequence=["#6366f1"])
        fig4.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig4, use_container_width=True)

    st.subheader("Message Activity This Session")
    all_entries = [
        (e["ts"], e["tone"])
        for entries in st.session_state.msg_log.values()
        for e in entries
    ]
    if all_entries:
        log_df = pd.DataFrame(all_entries, columns=["Time", "Tone"])
        st.dataframe(log_df, use_container_width=True, hide_index=True)
    else:
        st.caption("Generate messages to see activity here.")